...
function dbToCharacter(row: Record<string, unknown>): SavedCharacter {
  return {
    ...
    experiencePoints: (row.experience_points as number) || 0,
    subclass: (row.subclass as string) || "",
    subrace: (row.subrace as string) || "",
    armorClass: (row.armor_class as number) || 0,
    speed: (row.speed as number) || 0,
    hitDice: (row.hit_dice as string) || "",
    currentHitPoints: (row.current_hit_points as number) || 0,
    temporaryHitPoints: (row.temporary_hit_points as number) || 0,
    deathSaves: (row.death_saves as Character["deathSaves"]) || DEFAULT_CHARACTER.deathSaves,
    savingThrowProficiencies: (row.saving_throw_proficiencies as string[]) || [],
    skillProficiencies: (row.skill_proficiencies as string[]) || [],
    ...
    attacks: (row.attacks as Character["attacks"]) || [],
    spellcasting: (row.spellcasting as Character["spellcasting"]) || null,
    currency: (row.currency as Character["currency"]) || DEFAULT_CHARACTER.currency,
